...
2. 点击 `高级` > `调用合约`，输入旧合约的 ScriptHash, 可以查看到已部署的旧合约信息。
   ...
   ![migrate](https://github.com/BhpAlpha/docs/raw/master/asset/migrate.png)

3. 点击 `调用`，输入调用 Migrate 接口所需要的参数。注意第一个参数输入之前复制的新合约脚本。

   ![migrate2](https://github.com/BhpAlpha/docs/raw/master/asset/migrate2.png)

4. 输入完成后点击 `试运行` 查看结果，可以看到返回1-True，手续费为 491 GAS。

   ![migrate3](https://github.com/BhpAlpha/docs/raw/master/asset/migrate3.png)
